...
// 1. TCP SERVER
// ==========================================
let mayurHealthLoopStarted = false;

function querySingleMayurPanelHealth(panelId, ip) {
  return new Promise((resolve) => {
    console.log(`\n📡 [MAYUR HEALTH LOOP] Polling Panel #${panelId} at IP: ${ip}:${TCP_PORT}...`);
    ...
    const finish = (reason) => {
      if (!completed) {
        ...
        console.log(`🏁 [MAYUR HEALTH LOOP] Completed Panel #${panelId} (${reason})`);
        resolve();
      }
    };

    const timer = setTimeout(() => finish("timeout 15s"), 15000);

    socket.connect(TCP_PORT, ip, () => {
      console.log(`✅ [MAYUR HEALTH LOOP] Connected to Panel #${panelId} (${ip})`);
...
async function startPanelHealthLoop() {
  if (mayurHealthLoopStarted) return;
  mayurHealthLoopStarted = true;
  console.log("\n🔄 [MAYUR] Starting Continuous Sequential Panel Health Polling Loop...");

  while (true) {
    try {
      const [rows] = await pool.query(
        "SELECT NewPanelID, dvrip FROM sites WHERE Panel_Make LIKE '%mayur%' AND dvrip IS NOT NULL AND dvrip != '' AND TRIM(dvrip) != ''"
      );

      if (rows && rows.length > 0) {
        console.log(`\n📋 [MAYUR LOOP] Found ${rows.length} MAYUR panel(s) in 'sites' table. Polling one by one...`);
        for (let i = 0; i < rows.length; i++) {
          const panelId = String(rows[i].NewPanelID).trim();
          const ip = String(rows[i].dvrip).trim();

          console.log(`\n⏳ [MAYUR LOOP] [${i + 1}/${rows.length}] Processing Panel #${panelId} (${ip})...`);
          await querySingleMayurPanelHealth(panelId, ip);
          await new Promise(r => setTimeout(r, 2000));
        }
      } else {
        console.log(`ℹ️ [MAYUR LOOP] No active MAYUR panels found in 'sites' table. Waiting 10s...`);
        await new Promise(r => setTimeout(r, 10000));
      }
    } catch (err) {
      console.error(`❌ [MAYUR LOOP] Error during loop execution:`, err.message);
...
function startServer(customPort) {
  ...
  tcpServer.listen(port, () => {
    console.log(`🚀 MAYUR TCP Server listening for devices on port ${port}`);
  });
}
